...
class Freetype < Package
  ...
  def self.build
    system "sudo sed -i  -e \"/AUX.*.gxvalid/s@^# @@"+" -e"+"/AUX.*.otvalid/s@^# @@\" modules.cfg && sudo sed -ri -e 's:.*(#.*SUBPIXEL.*) .*:\1:' include/config/ftoption.h && sudo ./configure --prefix=/usr --disable-static CC=\"gcc -m32\""
    system "sudo make"
  end
